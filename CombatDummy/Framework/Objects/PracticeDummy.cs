...
namespace CombatDummy.Framework.Objects
{
    internal class PracticeDummy
    {
        ...
        public static void Draw(Object __instance, SpriteBatch spriteBatch, int x, int y, float alpha)
        {
            ...
            spriteBatch.Draw(CombatDummy.assetManager.PracticeDummyTexture, destination, new Rectangle(animationFrame * 16, 0, 16, 32), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, __instance.GetBoundingBox().Bottom / 10000f);
            ...
            if (collectiveDamage > 0 && adjustedAlpha > 0)
            {
                ...
                SpriteText.drawStringHorizontallyCenteredAt(spriteBatch, collectiveDamage.ToString(), (int)position.X + 32, (int)position.Y - 16 - (yOffset * 2), color: SpriteText.color_Red, alpha: adjustedAlpha);
            }
        }
    }
}
